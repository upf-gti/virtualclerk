...
function FacialController(o) {
    ...
    */
    ...
    else if(!this._gazePositions["HEAD"])
    {
      var headNode = LS.GlobalScene.getNodeByUId(this.headNode)
          this._gazePositions["HEAD"] = headNode.transform.globalPosition;
     
    }
    LS.GlobalScene.getActiveCameras(true);
    if (LS.GlobalScene._cameras[0])
        this._gazePositions["CAMERA"] = LS.GlobalScene.getCamera().getEye();
    else
    ...
    if (!this.lookAtEyes) 
      ...
      this._gazePositions["EYESTARGET"] = lookAtEyesNode.transform.position;
    
    if (!this.lookAtHead) 
      console.error("LookAt Head not found");
    else if( !this._gazePositions["HEADTARGET"]) 
      this._gazePositions["HEADTARGET"] = lookAtHeadNode.transform.position;
    
    if (!this.lookAtNeck) 
      console.error("LookAt Neck not found");
    else if( !this._gazePositions["NECKTARGET"] )
      this._gazePositions["NECKTARGET"] = lookAtNeckNode.transform.position;
  ...
  FacialController.prototype.onUpdate = function(e,dt)
  {
    ...
    {
        this._gazePositions["CAMERA"] = LS.GlobalScene.getCamera().getEye();
      this._gazePositions["CAMERA"][2] = 400;
    }
    // Gaze
    if (this.gazeManager){
  ...
  FacialController.prototype.newTextToLip = function (info)
  {
    if(!this.lipsync){ // setup
      ...
      let BS = this._morphDeformers["Body_SSS"]|| this._morphDeformers["Body"];
      let t2lBSWMap = T2LTABLES.BlendshapeMapping;
  
      for(let i = 0; i<BS.length; i++)
      {
        if(BS[i].mesh.includes("Midmouth_Left"))      this.lipsyncBSMapping.push( [ i, t2lBSWMap.kiss, 0.4 ]);
        if(BS[i].mesh.includes("Midmouth_Right"))     this.lipsyncBSMapping.push( [ i, t2lBSWMap.kiss, 0.4 ]);
        if(BS[i].mesh.includes("MouthNarrow_Left"))   this.lipsyncBSMapping.push( [ i, t2lBSWMap.kiss, 1.0 ]);
        if(BS[i].mesh.includes("MouthNarrow_Left"))   this.lipsyncBSMapping.push( [ i, t2lBSWMap.kiss, 1.0 ]);
  
        if(BS[i].mesh.includes("MouthDown"))          this.lipsyncBSMapping.push( [ i, t2lBSWMap.upperLipClosed, 0.4 ]);
        if(BS[i].mesh.includes("UpperLipOut"))        this.lipsyncBSMapping.push( [ i, t2lBSWMap.upperLipClosed, -1.5 ]);
        if(BS[i].mesh.includes("UpperLipUp_Left"))    this.lipsyncBSMapping.push( [ i, t2lBSWMap.upperLipClosed, -0.3 ]);
        if(BS[i].mesh.includes("UpperLipUp_Right"))   this.lipsyncBSMapping.push( [ i, t2lBSWMap.upperLipClosed, -0.3 ]);
  
        if(BS[i].mesh.includes("LowerLipDown_Left"))  this.lipsyncBSMapping.push( [ i, t2lBSWMap.lowerLipClosed, -0.8 ]);
        if(BS[i].mesh.includes("LowerLipDown_Right")) this.lipsyncBSMapping.push( [ i, t2lBSWMap.lowerLipClosed, -0.8 ]);
        if(BS[i].mesh.includes("LowerLipIn"))         this.lipsyncBSMapping.push( [ i, t2lBSWMap.lowerLipClosed, 1.0 ]);
  
        if(BS[i].mesh.includes("MouthOpen"))          this.lipsyncBSMapping.push( [ i, t2lBSWMap.jawOpen, 1.0 ]);
  
        if(BS[i].mesh.includes("TongueBackUp"))       this.lipsyncBSMapping.push( [ i, t2lBSWMap.tongueBackUp,  1.0 ]);
        if(BS[i].mesh.includes("TongueFrontUp"))      this.lipsyncBSMapping.push( [ i, t2lBSWMap.tongueFrontUp, 1.0 ]);
        if(BS[i].mesh.includes("TongueOut"))          this.lipsyncBSMapping.push( [ i, t2lBSWMap.tongueOut,     1.0 ]);
      }
    }// end of setup
  ...
  FacialController.prototype.facialBlend = function(dt)
  {
    ...
    if(this.lipsync && this.lipsync.getCompactState() == 0 ) // when getCompactState==0 lipsync is working, not paused and has sentences to process
    {
      ...
      {    
        ...
        for(let i = 0; i < this.lipsyncBSMapping.length; i++)
        {
          ...
          BS[ mapping[0] ].weight = Math.min( 1, Math.max( -1, t2lBSW[ mapping[1] ] * mapping[2] ) );
        }
      }
      console.log(t2lBSW);
    ...
    //if(blinkW && this._Blink && this._blinking) this._Blink.time = this._Blink.end;
  ...
  */
  ...
  this._gazePositions = { "RIGHT": [70, 100, 400], "LEFT": [-70, 100, 400],
    "UP": [0, 130, 400], "DOWN": [0, 80, 400],
    "UPRIGHT": [70, 130, 400], "UPLEFT": [-70, 130, 400],
    "DOWNRIGHT": [70, 80, 400], "DOWNLEFT": [-70, 80, 400],
    "CAMERA": [0, 100, 400]
