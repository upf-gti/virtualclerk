...
async def language(request: Request):
    ...
    if GoogleTranslator().is_language_supported(language=txt):
        global lang
        lang = txt
...
async def transcribe(request: Request):
    ...
    result = model.transcribe('test.wav', language = lang, task="translate", fp16=False)
    ...
    txt = transcription
    if lang != 'en':
        txt = GoogleTranslator(source=lang, target="en").translate(txt)
    #Contact RASA
    headers = {
    ...
    prefix = "./audios/response{}".format(rand)
...
async def text(request: Request):
    ...
    os.remove(filepath)
    print(audio)
    response_data = MyResponse(query_transcription=otext, lang='', response_transcription=text,
                               audio_path=filepath, rand=rand, randletter=randletter, audio=audio)
    response_json = jsonable_encoder(response_data)
