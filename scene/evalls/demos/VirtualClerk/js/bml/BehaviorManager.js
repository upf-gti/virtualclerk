...
// Message gets to BehaviorManager -> then to facial control -> then to BehaviorRealizer
...
BehaviorManager.prototype.removeBlockByType = function(type, time){

	// Several blocks can be active (MERGE composition)
	for (var i = 0; i<this.stack.length; i++)
	{    
		// If it is not active
		if (this.stack[i][type] && this.stack[i].isActive)
		{
			// Remove
			this.stack[i][type].endGlobalTime = this.time;
			return;
		}
	}
	// let idx = this.bmlKeys.indexOf(type);
...
BehaviorManager.prototype.update = function(actionCallback, time){
...
BehaviorManager.prototype.newBlock = function(block, time){
...
BehaviorManager.prototype.fixBlock = function(block){
...
BehaviorManager.prototype.fixBML = function(bml, key, block, sync){
	// Error check
	if (!bml)
	{
...
BehaviorManager.prototype.checkSync = function(syncAttr, block, it){
...
BehaviorManager.prototype.findTime = function(id, syncName, block, it){
...
BehaviorManager.prototype.findEndOfBlock = function(block){
...
BehaviorManager.prototype.addToStack = function(block){
	...
	{ 
		// Doens't make sense, only for individual stacks, not whole
		// Substitute in stack
	...
	else
	{
		...
		{ 
			// Try to merge, if not, add "del" variable to bml
			if (block.blink)
...
BehaviorManager.prototype.removeFromStacks = function(block){
	
	var keys = Object.keys(block);
	// Add delete variable in block to bml instructions
	for (var i = 0; i < keys.length; i++)
	{ // Through bml instructions
		var bml = block[keys[i]];
	...
	for (var i = 0; i<this.BMLStacks.length; i++)
	{ // Through list of stacks
		for (var j = 0; j<this.BMLStacks[i].length; j++) 
		{// Through stack
			if (this.BMLStacks[i][j].del)
			{ // Find del variable in stack
...
BehaviorManager.prototype.cleanBlock = function(block){
...
BehaviorManager.prototype.addToBMLStack = function(block){
...
BehaviorManager.prototype.processIntoBMLStack = function(bml, stack, globalStart, composition){
	...
	let merged = this.mergeBML(bml,stack,globalStart, overwrite, merge);
...
BehaviorManager.prototype.mergeBML = function(bml, stack, globalStart, overwrite, merge = false){
...
BehaviorManager.prototype.mergeBMLSyncFix = function(syncAttr, start, globalStart){
...
BehaviorManager.prototype.check = function(){
...
// Provide feedback
BehaviorManager.prototype.progressFeedback = function(id, sync, time){
